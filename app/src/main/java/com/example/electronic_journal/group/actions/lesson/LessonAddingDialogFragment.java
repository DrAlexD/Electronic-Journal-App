...
public class LessonAddingDialogFragment extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        RadioButton isLecture = root.findViewById(R.id.lessonIsLectureAdding);
        RadioButton isSeminar = root.findViewById(R.id.lessonIsSeminarAdding);
        if (professorType == 2) {
        ...
        TextWatcher afterTextChangedListener = new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                lessonAddingViewModel.lessonAddingDataChanged(dateAndTime.getText().toString(), pointsPerVisit.getText().toString(),
                        lessonAddingViewModel.getModule().getValue() != null ?
                                lessonAddingViewModel.getModule().getValue().getSubjectInfo().getSemester() : null);
            }
        ...
        dialog.getButton(Dialog.BUTTON_POSITIVE).setOnClickListener(view -> {
            ...
            answerLiveData.observe(getParentFragment().getViewLifecycleOwner(), answer -> {
                if (answer != null) {
                    ...
                    Navigation.findNavController(getParentFragment().getView()).navigate(R.id.action_dialog_lesson_adding_to_group_performance_lessons, bundle);
                }
            });
        });
    }
